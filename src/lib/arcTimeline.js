...
/** Inclusive calendar range for a forge_block. */
export function getBlockDateRange(block) {
  const start = block?.startDate;
  if (!start) return null;
  const duration = getArcDurationDays(block);
  const end = block.endDate || ymdAddDays(start, duration - 1);
  return { start, end };
}

/** Which Arc owns a journal date (first match in chronological block order). */
export function findOwningBlockForDate(dateStr, blocks) {
  if (!dateStr) return null;
  const sorted = [...(blocks || [])]
    .filter(b => b?.id && b?.startDate)
    .sort((a, b) => (a.startDate || "").localeCompare(b.startDate || ""));
  for (const block of sorted) {
    const range = getBlockDateRange(block);
    if (!range) continue;
    if (dateStr >= range.start && dateStr <= range.end) return block;
  }
  return null;
}

function fmtShortDateRange(startYmd, endYmd) {
  const s = parseLocal(startYmd);
  const e = parseLocal(endYmd);
...
/** Headline for neutral (non-Arc) evidence chapters — no AI at render time. */
export function deriveNeutralHeadline(entries, kind = "pre") {
  ...
  for (const e of sorted) {
    const first = stripJournalTitleLine((e.content || "").split("\n")[0] || "");
    if (first?.trim()) return first.trim();
  }
  if (kind === "between") return "Between Arcs";
  if (kind === "after") return "After your last Arc";
  return "Evidence before this Arc";
}

const NEUTRAL_KIND_LABEL = {
  pre: "Before your first Arc",
  between: "Between Arcs",
  after: "After your last Arc",
};

/** Group unassigned journal entries by calendar week (Mon–Sun). */
export function groupEntriesByCalendarWeek(entries, today = todayStr()) {
  ...
  const weeks = [];
  for (const [weekStart, ents] of byWeek) {
    ...
    weeks.push({
      ...
      rangeLabel: fmtShortDateRange(weekStart, weekEnd),
    });
  }
...
/**
 * Continuous personal history: neutral weeks, completed Arcs, active Arc — chronological.
 * Journal entries attach to the Arc whose date range contains them; everything else is neutral.
 */
export function buildLifeChronology({ blocks = [], journalEntries = [], today = todayStr() }) {
  const sortedBlocks = [...blocks]
    .filter(b => b?.id && b?.startDate)
    .sort((a, b) => (a.startDate || "").localeCompare(b.startDate || ""));

  const entriesByBlockId = new Map();
  for (const b of sortedBlocks) entriesByBlockId.set(b.id, []);

  const neutralPool = [];
  for (const e of journalEntries || []) {
    if (!e?.date || e.date > today) continue;
    const owner = findOwningBlockForDate(e.date, sortedBlocks);
    if (owner) entriesByBlockId.get(owner.id).push(e);
    else neutralPool.push(e);
  }

  const segments = [];
  let prevArcEnd = null;

  const pushNeutralWeeks = (entries, kind) => {
    const weeks = groupEntriesByCalendarWeek(entries, today);
    for (const w of weeks) {
      segments.push({
        type: "neutral",
        id: `neutral-${kind}-${w.weekStart}`,
        kind,
        kindLabel: NEUTRAL_KIND_LABEL[kind] || NEUTRAL_KIND_LABEL.pre,
        headline: deriveNeutralHeadline(w.entries, kind),
        ...w,
      });
    }
  };
  ...
  for (const block of sortedBlocks) {
    ...
    const gapEntries = neutralPool.filter(e => {
      if (prevArcEnd) return e.date > prevArcEnd && e.date < range.start;
      return e.date < range.start;
    });
    if (gapEntries.length > 0) {
      pushNeutralWeeks(gapEntries, prevArcEnd ? "between" : "pre");
    }

    const isActive = block.status === "active";
    segments.push({
      type: isActive ? "active-arc" : "completed-arc",
      id: `arc-${block.id}`,
      block,
      journalEntries: entriesByBlockId.get(block.id) || [],
    });

    prevArcEnd = range.end;
  }

  const hasActive = sortedBlocks.some(b => b.status === "active");
  if (!hasActive && prevArcEnd) {
    const afterEntries = neutralPool.filter(e => e.date > prevArcEnd);
    if (afterEntries.length > 0) pushNeutralWeeks(afterEntries, "after");
  }

  if (sortedBlocks.length === 0 && neutralPool.length > 0) {
    pushNeutralWeeks(neutralPool, "pre");
  }

  return {
    segments,
    activeBlock: sortedBlocks.find(b => b.status === "active") || null,
  };
}
