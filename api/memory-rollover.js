    ...
    const resp = await client.messages.create({
      ...
      system: [{
        ...
        text:
          ...
          "- structured: { \"narrative\": string|null, \"wins\": [..], \"hard_parts\": [..], \"slips\": [..], \"mood\": string|null, \"tomorrow_focus\": string|null }. `narrative` is the most important field here — it gets shown VERBATIM as the opening line of tomorrow's morning greeting, the very first thing the user reads that day. Write ONE to TWO flowing sentences in second person (\"You wrapped up the Companion redesign, played poker with the guys, and pushed the Azir meeting to next week.\") that read like a friend recapping your day back to you — never a list, never bullet fragments stitched together with commas, never a field-by-field report. Name the real events, projects, and people involved. If genuinely nothing happened worth recapping, use null rather than forcing something. `wins`/`hard_parts`/`slips` are still extracted the same way as before (short natural phrases, empty arrays when nothing fits) for other surfaces — but `narrative` is what the user actually sees first, so it carries the most weight of any field here.\n" +
