...
**Last updated:** 2026-07-05 — a follow-up round after real testing of the previous round's fixes surfaced a new, more specific audio symptom (see item 0 below), plus the two earlier same-day fixes:

0. **NEW — audio was stuttering every couple of words ("a couple words, cut out, a couple words, cut out"), on top of a still-present multi-second startup delay.** Root cause: the *previous* round's AudioContext fix (item 1 below) resolved "no sound at all" by routing the shared `<audio>` element through `createMediaElementSource()`, but that had a side effect nobody had hit yet — once an element's output is permanently rerouted through a Web Audio graph, reassigning that element's `.src` (which `useCoachTts.jsx` does once per sentence-chunk, by design) forces the browser to reload/resync the whole graph, and that reload is audible as a brief glitch. With chunk-per-sentence playback, that meant a glitch at *every single chunk boundary*, not just some — exactly "a couple words, cut out, repeat." Fixed by removing the `<audio>` element from the TTS path entirely: `useCoachTts.jsx` now decodes each chunk straight to a Web Audio `AudioBuffer` (`ctx.decodeAudioData`) and schedules each one with `AudioBufferSourceNode.start(time)` timed to begin exactly when the previous chunk's buffer ends (on the `AudioContext`'s own clock) — genuinely gapless, with no element-reload step to glitch on. The Ember's audio-reactive visualizer now reads frequency data straight off this same hook's analyser node instead of creating its own separate `AudioContext`/`MediaElementSource` in `CompanionScreen.jsx` (removed — one audio pipeline now, not two). **Unverified by ear in this sandbox** — reasoned carefully from how `HTMLMediaElement` + Web Audio routing is documented to behave, but needs a real listen to confirm the stutter is actually gone and hasn't been replaced by something else. The remaining "few seconds before speech starts" delay is very likely ElevenLabs' own synthesis time for the first chunk (not fixable from this side without changing what's fetched/when) — not yet separately investigated; worth a follow-up if it's still bothersome after confirming the stutter is fixed.
1. **Audio was still silent even after the concurrency fix, with no error shown at all** — a genuinely different bug from every previous one this session, and probably the actual root cause behind "no audio" the whole time. `CompanionScreen.jsx`'s Web Audio analyser setup (the thing that makes the Ember visually react to the AI's voice) creates an `AudioContext` and reroutes the `<audio>` element's entire output through it via `createMediaElementSource()`. New `AudioContext`s commonly start `'suspended'` (especially iOS Safari) — and once an element's output is rerouted through a suspended context, **that element is permanently silent from then on**, even though `.play()` still resolves fine and the element still reports "playing." Every previous fix this session (fetch latency, `Content-Length`, concurrency) was real and worth keeping, but none of them could have mattered if this was true underneath the whole time. Fixed by creating/resuming the `AudioContext` as early as possible — synchronously inside the actual tap/send gesture, not only once a reply finishes and TTS playback begins — and re-attempting `.resume()` every time speaking starts. **Superseded by item 0 above** — the `<audio>` element this fix was resuming is now gone entirely; the AudioContext-priming-on-gesture principle itself is preserved (`coachTts.primeAudio()` is still called synchronously from tap/send handlers). See §5.
2. **The coach's personality is too rigidly fixated on the Arc/proof actions/goals**, even in "Just chat" mode, refusing simple requests ("tell me a joke" → "Not my lane, mate. I'm here to track whether you hit your proof actions...") instead of just engaging like a normal AI assistant. Added an explicit, forceful counter-instruction to the shared response-style steering and hardened "Just chat" specifically to switch that instinct off entirely unless the user brings up goals themselves. **Important limit on this fix, stated plainly:** the underlying fixation almost certainly comes from the base coach personality in the protected `AICoach.jsx`/`buildCoachSystemPrompts` — this fix layers a strong counter-instruction on top of that (in the non-protected `CompanionScreen.jsx`), which should meaningfully help, but can't guarantee it fully overrides a personality trait that's this deliberately, heavily emphasized at the source. If it's still not loose enough after testing, the real fix requires touching the protected file itself, which needs your explicit sign-off, not something to do unilaterally. **Confirmed working in real testing** — the most recent screenshot showed an actual, unredirected joke in "Just chat" mode. Not formally exhaustively re-tested across every mode, but the fix appears to be holding.
