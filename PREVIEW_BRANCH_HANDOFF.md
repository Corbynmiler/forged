...
**Last updated:** 2026-07-05 — found and fixed the actual root cause of the recurring "audio pauses/cuts out, especially at full stops, doesn't feel continuous" complaint: `useCoachTts.jsx`'s per-sentence-chunk prefetching was staggered (only start fetching chunk N+1 once chunk N's own fetch resolves), which gives each next chunk exactly the current chunk's *playback duration* to finish downloading — not enough margin for a short sentence. Changed to fire every chunk's fetch in parallel immediately, so later chunks get the cumulative head start of everything already spoken, not just one chunk's worth. This is very likely what the last two rounds of "cutting out" reports were actually describing — not stream truncation (which was a real, separate, smaller issue, now also fixed). See §5.

**Also this round:** fixed a self-introduced regression from the round before — the "buffer audio server-side before sending" change (meant to fix mid-reply stream truncation) shipped without an explicit `Content-Length` header, which broke audio playback entirely for one test (silent failure on one attempt, a client-side "Playback failed" error on another). Fixed by explicitly setting `Content-Length` and rejecting empty buffers before sending.
...
- **Round 2, the actual dominant cause — inter-chunk gaps, found after "cuts out especially at full stops, doesn't feel continuous" made the pattern clear:** `useCoachTts.jsx` speaks a reply one sentence-chunk at a time, and was prefetching *staggered* — chunk N+1's fetch only started once chunk N's own fetch resolved, racing it against chunk N's playback. That gives chunk N+1 exactly chunk N's playback duration to finish downloading — for a short sentence (well under a second to speak), that's often not enough time for a real network+synthesis round trip, producing an audible gap right at the sentence boundary. **Fixed by firing every chunk's fetch in parallel immediately**, instead of staggered — so chunk 4, for example, gets the cumulative playback time of chunks 0-3 as head start, not just chunk 3's. This is very likely what both rounds of "cutting out" feedback were actually describing, more than the stream-truncation theory above.
...
- **Minor known tradeoff from the parallel-fetch fix:** firing all chunks at once means if an early chunk hits the monthly character quota mid-reply, a couple of later chunks may have already been in flight (and briefly counted against usage) before the loop stops playing further chunks — a small, personal-quota-only race, not a security concern, not fixed this round since it's a minor cost for a much larger continuity win.
...
- **Still unverified:** whether the parallel-fetch fix actually closes the gaps (this is the one that matters most), whether the latency fix meaningfully shortened the delay before speech starts, and whether "George" is genuinely better than "Adam" — all need your ears, not mine.
