...
**Last updated:** 2026-07-05 — three rounds on the same bug in quick succession, each one correcting the last. Root cause of "audio pauses/cuts out, especially at full stops, doesn't feel continuous": `useCoachTts.jsx`'s per-sentence-chunk prefetching was staggered (chunk N+1 only started fetching once chunk N's own fetch resolved, then raced chunk N's playback) — not enough lead time for a short sentence. First attempted fix (fire every chunk's fetch in parallel, immediately) made it *worse* — it exceeded ElevenLabs' per-account concurrent-request limit (free tier: 2; Starter: 3) and got requests rejected outright ("Could not generate speech right now," no audio at all). **Corrected to a bounded sliding window of 2 concurrent fetches** — safely within even the free tier's limit, while still giving every chunk beyond the first two a real head start over the original staggered bug. Verified the windowing logic itself with a standalone simulation (confirms concurrency never exceeds 2, for any chunk count) — verifying the actual listening experience still needs your ears. See §5.

**Also earlier this session:** fixed a self-introduced regression — the "buffer audio server-side before sending" change (meant to fix mid-reply stream truncation) shipped without an explicit `Content-Length` header, which broke audio playback entirely for one test. Fixed by explicitly setting `Content-Length` and rejecting empty buffers before sending. That fix is unrelated to the concurrency issue above and is still in place.
...
- **If gaps are still audible after this fix:** the next lever is raising `CONCURRENT_TTS_FETCH_WINDOW` in `useCoachTts.jsx` from 2 to 3 — but only if this ElevenLabs account is confirmed to be on at least the Starter plan (3 concurrent requests), otherwise that would reintroduce the exact regression just fixed. Confirm the plan tier before touching that constant.
...
- **Still unverified:** whether the corrected windowed-prefetch fix actually closes the gaps to something that sounds continuous (this is the one that matters most), whether the latency fix meaningfully shortened the delay before speech starts, and whether "George" is genuinely better than "Adam" — all need your ears, not mine.
